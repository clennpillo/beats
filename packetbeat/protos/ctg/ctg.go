...
var (
	unmatchedResponses = expvar.NewInt("ctg.unmatched_responses")
)
...
func (ctg *ctgPlugin) init(results publish.Transactions, config *ctgConfig) error {
	ctg.setFromConfig(config)
...
func (ctg *ctgPlugin) setFromConfig(config *ctgConfig) {
	ctg.ports = config.Ports
	ctg.sendRequest = config.SendRequest
	ctg.sendResponse = config.SendResponse
	ctg.hideKeywords = config.HideKeywords
	ctg.redactAuthorization = config.RedactAuthorization
	ctg.splitCookie = config.SplitCookie
	ctg.parserConfig.realIPHeader = strings.ToLower(config.RealIPHeader)
	ctg.transactionTimeout = config.TransactionTimeout
	ctg.includeBodyFor = config.IncludeBodyFor
	ctg.maxMessageSize = config.MaxMessageSize

	if config.SendAllHeaders {
		ctg.parserConfig.sendHeaders = true
		ctg.parserConfig.sendAllHeaders = true
	} else {
		if len(config.SendHeaders) > 0 {
			ctg.parserConfig.sendHeaders = true

			ctg.parserConfig.headersWhitelist = map[string]bool{}
			for _, hdr := range config.SendHeaders {
				ctg.parserConfig.headersWhitelist[strings.ToLower(hdr)] = true
			}
		}
	}
}
...
func (ctg *ctgPlugin) GetPorts() []int {
	return ctg.ports
}
...
func (ctg *ctgPlugin) messageGap(s *stream, nbytes int) (ok bool, complete bool) {
...
// of a message.
func (ctg *ctgPlugin) messageComplete(
...
func (ctg *ctgPlugin) doParse(
	...
	if st == nil {
		...
		// concatenate bytes
		if len(st.data)+len(pkt.Payload) > ctg.maxMessageSize {
			if isDebug {
...
func (ctg *ctgPlugin) ReceivedFin(tcptuple *common.TCPTuple, dir uint8,
...
// to packet loss).
func (ctg *ctgPlugin) GapInStream(tcptuple *common.TCPTuple, dir uint8,
...
func (ctg *ctgPlugin) correlate(conn *ctgConnectionData) {
...
func (ctg *ctgPlugin) newTransaction(requ, resp *message) common.MapStr {
...
func (ctg *ctgPlugin) publishTransaction(event common.MapStr) {
	if ctg.results == nil {
		return
	}
...
func (ctg *ctgPlugin) RemovalListener(data protos.ProtocolData) {
...
func (ctg *ctgPlugin) collectHeaders(m *message) interface{} {
	...
	if ctg.parserConfig.sendHeaders {
		...
		for name, value := range m.headers {
			...
			if ctg.splitCookie && name == cookie {
...
func (ctg *ctgPlugin) setBody(result common.MapStr, m *message) {
	body := string(ctg.extractBody(m))
	if len(body) > 0 {
		result["body"] = body
	}
}
...
func (ctg *ctgPlugin) extractBody(m *message) []byte {
	body := []byte{}
...
func (ctg *ctgPlugin) cutMessageBody(m *message) []byte {
	cutMsg := []byte{}
	...
	// add body
	return append(cutMsg, ctg.extractBody(m)...)

}

func (ctg *ctgPlugin) shouldIncludeInBody(contenttype []byte) bool {
	includedBodies := ctg.includeBodyFor
...
func (ctg *ctgPlugin) hideHeaders(m *message) {
...
func (ctg *ctgPlugin) hideSecrets(values url.Values) url.Values {
	params := url.Values{}
...
func (ctg *ctgPlugin) extractParameters(m *message, msg []byte) (path string, params string, err error) {
...
func (ctg *ctgPlugin) isSecretParameter(key string) bool {
