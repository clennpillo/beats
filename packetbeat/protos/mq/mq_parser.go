...
func (parser *parser) parseMqSign(s *stream, m *message) (cont, ok, complete bool) {
	...
	if bytes.Equal(s.data[9:10], []byte("\x96")) || //MQPUT_REPLY
	   bytes.Equal(s.data[9:10], []byte("\x95")) {  //MQGET_REPLY
	    ...
	    if bytes.Equal(s.data[9:10], []byte("\x96")) {
	    	...
	    	m.cbodData = lib.ParseCBODOut(s.data[612:len(s.data)])
	    }
		...
		if bytes.Equal(s.data[9:10], []byte("\x86")) {
			...
			m.cbodData = lib.ParseCBODIn(s.data[640:len(s.data)])
	    }else{
	    	m.MqType = "MQGET"
            debugf("MQGET")
	    }
